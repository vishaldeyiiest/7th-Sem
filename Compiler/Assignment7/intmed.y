...
loop:	
	...
	WHILE_TOK '(' expr ')' stmt	
	{
		char *t1 = newLabel();
		char *t2 = newLabel();
		$$ = (char *)malloc((strlen($3.code) + strlen($5) + strlen($3.place) + 2* strlen(t1) + 2* strlen(t2) + 28) * sizeof(char));
		char *ss = (char*)malloc(strlen($3.code) + 3);
		strcpy(ss, ": ");
		int i = 0, k = 2;
		for(i = 1; i < strlen($3.code); i++)
			ss[k++] = $3.code[i];
		//strcat(ss, $3.code);		
...
cond:		
	IF_TOK '(' expr ')' stmt 
		{
			char * tmp = newLabel();
			$$ = (char *)malloc((strlen($3.code) + strlen($5) + strlen($3.place) + 2 * strlen(tmp) + 19) * sizeof(char));
	...
	| IF_TOK '(' expr ')' stmt ELSE_TOK stmt
	{
		char * tmp1 = newLabel();
		//char * tmp2 = newLabel();
		$$ = (char *)malloc((strlen($3.code) + strlen($5) + strlen($7) + strlen($3.place) + 2* strlen(tmp1) + 20) * sizeof(char));
		strcpy($$, $3.code);
		strcat($$, "\nIF NOT ");
		strcat($$, $3.place);
		strcat($$, " GOTO ");
...
expression codegen(expression a, char *op, expression b)
{
	...
	if(a.code[0] != '\0')
		strcpy(ret.code, a.code), strcat(ret.code, "\n");
	if(b.code[0] != '\0')
		strcat(ret.code, b.code), strcat(ret.code, "\n");
...
expression codegenAssign(char *a,  expression c)
{
	...
	if(c.code[0] != '\0')
		strcpy(buffer_quad.code, c.code), strcat(buffer_quad.code, "\n");
